...
(function() {
  var Accordion, Dropdown, Offcanvas, Readmore, Slider, Tab, Topbar,
  ...
  Readmore = (function(superClass) {
    extend(Readmore, superClass);
    ...
    function Readmore(el) {
      ...
      if ($.trim(this.$el.text()) === '') {
        return;
      }
      if (!(this.$el.hasClass('readmore--opened') || this.$el.hasClass('readmore--closed'))) {
        this.$el.addClass('readmore--closed');
      }
      this.button = this.$el.next();
      if (!this.button.hasClass('readmore__button')) {
    ...
    Readmore.prototype.toggle = function(event) {
      this.$el.slideToggle((function(_this) {
        return function() {
          _this.$el.toggleClass('readmore--opened readmore--closed');
          _this.button.text(_this.buttonText());
          _this.$el.css('display', "");
          if (_this.$el.attr('style') === '') {
            _this.$el.removeAttr('style');
          }
          if (_this.$el.hasClass('readmore--opened')) {
            return _this.trigger('open', _this.$el);
          } else {
            return _this.trigger('close', _this.$el);
          }
        };
      })(this));
      return false;
    ...
    Readmore.prototype.buttonState = function() {
      if (this.$el.hasClass('readmore--opened')) {
        return 'close';
      }
      return 'open';
    };

    Readmore.prototype.buttonText = function(state) {
      if (state == null) {
    ...
    Readmore.prototype.renderButton = function() {
      var text;
      text = this.buttonText();
      return $('<a></a>').addClass('readmore__button').attr('href', '#').html(text);
    };
  ...
  Topbar = (function(superClass) {
    ...
    Topbar.prototype.toggle = function(e) {
      return this.$el.toggleClass('topbar--open');
    };

    Topbar.prototype.toggleItem = function(e) {
      var item, subList;
  ...
  Accordion = (function(superClass) {
    ...
    function Accordion(el) {
      ...
      this.$el.find('.accordion__title').on('click', this.toggleItem);
      this.items = this.$el.find('.accordion__item');
      this.offset = this.$el.data('scroll-offset');
    }

    Accordion.prototype.toggleItem = function(e) {
      var currentOpen, item;
      item = $(e.target).closest('.accordion__item');
      currentOpen = item.hasClass('accordion__item--open');
      this.items.filter('.accordion__item--open').each((function(_this) {
        return function(_, i) {
          return _this.trigger('close', $(i));
        };
      })(this));
      this.items.removeClass('accordion__item--open');
      if (!currentOpen) {
        item.toggleClass('accordion__item--open');
        this.trigger('open', item);
      }
      e.preventDefault();
      return Partystreusel.scrollTo(item, this.offset);
    };
  ...
  Tab = (function(superClass) {
    ...
    function Tab(el) {
      this.openItem = bind(this.openItem, this);
      Tab.__super__.constructor.apply(this, arguments);
      this.$el.find('.tab__nav-item').on('click', this.openItem);
      this.$navItems = this.$el.find('.tab__nav-item');
      this.$panels = this.$el.find('.tab__panel');
      if (this.findItemAndPanel(window.location.hash)) {
    ...
    Tab.prototype.openItem = function(e) {
      var item, name;
    ...
    Tab.prototype.currentPanelName = function() {
      return this.$panels.filter('.tab__panel--active').attr('id');
    };

    Tab.prototype.openPanel = function(name_or_number) {
      var closeName, itemAndPanel, itemAndPanelToClose, panelName;
      if (typeof name_or_number === 'number') {
        panelName = this.$panels[name_or_number].id;
      } else {
        panelName = name_or_number;
      }
      itemAndPanel = this.findItemAndPanel(panelName);
      if (!itemAndPanel) {
        console.log("Nav Item or panel with name " + panelName + " not found");
        return;
      }
      itemAndPanelToClose = this.findItemAndPanel(this.currentPanelName());
      if (itemAndPanelToClose) {
        closeName = this.currentPanelName();
      ...
      return this.trigger('open', this.currentPanelName(), itemAndPanel[0], itemAndPanel[1]);
    };

    Tab.prototype.findItemAndPanel = function(name) {
      var navItem, panel;
      if (name == null) {
        return;
      }
      ...
      panel = this.$panels.filter("[id=" + name + "]");
      if (panel.length === 0) {
        return;
      }
      navItem = this.$navItems.filter(function(index, item) {
        return $(item).find("[href='\#" + name + "']").length > 0;
      });
      if (navItem.length === 0) {
        return;
      }
  ...
  Slider = (function(superClass) {
    ...
    function Slider(el) {
      ...
      this.slidelist = this.$el.find('.slider__list');
      this.initializeCycle();
    }

    Slider.prototype.initializeCycle = function() {
      return this.slidelist.cycle({
        slides: '.slider__item',
        fx: 'scrollHorz',
        speed: 300,
        timeout: 0,
  ...
  Partystreusel.scrollTo = function(x, relativeOffset) {
    var ref;
    ...
    if (typeof x !== 'number') {
      x = (ref = x.offset()) != null ? ref.top : void 0;
    }
    if (x == null) {
      return false;
    }
  ...
  Dropdown = (function(superClass) {
    ...
    function Dropdown(el) {
      ...
      this.$el.on('click', this.toggleDropdown);
    }
